...
1. **모든 릴리스는 직전 버전에서 인앱 업데이트로 도달 가능해야 한다.**
   ...
   설치파일(NSIS, v1.0.50부터 사이트 기본 다운로드)로 설치된 프로그램은 폴더형과 같은 구조라 zip 경로로 업데이트된다 — 별도 메커니즘 없음.

2. **릴리스 완료 보고 전 필수 검증 (하나라도 실패 시 릴리스 미완료):**
   - [ ] `latest.json`에 `url`+`sha256`(포터블), `zipUrl`+`zipSha256`(폴더형), `installerUrl`+`installerSha256`(설치파일) 모두 존재
   - [ ] 세 다운로드 URL 모두 HEAD 200 (`npm run verify:nas`)
...
13. **코드사이닝 인증서는 도입하지 않는다 (오너 결정 2026-09-07, 재제안 금지).** 미서명 exe의 백신 오탐은 다음으로 관리한다:
    ...
    배포 사이트 #antivirus 안내 / 오탐 발생 시 안랩·MS 오진 신고 + 실사고 기록. Azure Artifact Signing은 한국 미지원이라 대안이 아니다.
